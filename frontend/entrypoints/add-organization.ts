...
afterPageLoad().then(() => {
  ...
  saveButton.onclick = function () {
    ...
    sendCreateOrganizationRequest(name, description, active, showOnSignUp, orgParentID, countries, areas, municipalities).then((response: Response) => {
      if (response.ok) {
        window.location.href = 'organizations'
      } else {
        createMessage('Något gick fel när föreningen skulle skapas', 'is-danger', 'buttonGroup')
      }
    }).catch((reason) => {
      console.log(reason)
      createMessage('Något gick fel när föreningen skulle skapas', 'is-danger', 'buttonGroup')
